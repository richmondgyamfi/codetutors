...
class User
{
    ...
    public function gettotal_app_coun()
    {
        $this->db->query("SELECT (SELECT COUNT(*) FROM `counselors`) AS total_ap, 
        (SELECT COUNT(*) FROM `counselors` WHERE STATUS = 0 and LEFT(sub_date,10) > '2024-10-23') AS new_ap,
        (SELECT COUNT(*) FROM `counselors` WHERE STATUS = 1 and LEFT(sub_date,10) > '2024-10-23') AS selected_ap,
        (SELECT COUNT(*) FROM `counselors` WHERE STATUS = 2 and LEFT(sub_date,10) > '2024-10-23') AS rejected_ap ,
        (SELECT COUNT(*) FROM `counselors` WHERE STATUS = 3 and LEFT(sub_date,10) > '2024-10-23') AS appointed_ap
        FROM `counselors` where LEFT(sub_date,10) > '2024-10-23'");
    ...
    public function getall_app_coun()
    {
        if($_SESSION['centre'] != null || !empty($_SESSION['centre'])){
            $cont = $_SESSION['centre'];
            $this->db->query("SELECT * FROM `counselors` as t1 where LEFT(t1.sub_date,10) > '2024-10-23'");
...
left join osis.students_db as t2 on t1.studid = t2.studid where t1.status = 0 and LEFT(t1.sub_date,10) > '2024-10-23'");
...
left join hr.staff as t2 on t1.staff_id = t2.staff_no where t1.status = 0");
    ...
    {
        $runtype = (($_SESSION['role'] == 2) ? 'DC' : 'RG');
        $this->db->query("SELECT * FROM `counselors` where LEFT(sub_date,10) > '2024-10-23'");
    ...
    public function counsel_receivedall()
    {
        ...
        where t1.status > 0 and LEFT(t1.sub_date,10) > '2024-10-23'");
    ...
    public function counsel_received()
    {
        ...
        left join osis.students_db as t2 on t1.studid = t2.studid 
        left join hrmscode.users as t3 on t1.received_by = t3.username
        where t1.status > 0 and t1.received_by = :username and LEFT(t1.sub_date,10) > '2024-10-23'");
        $this->db->bind(':username', $_SESSION['username']);
    ...
    public function counsel_log($data)
    {
        ...
        left join hrmscode.users as t3 on t2.reassigned_to = t3.username where cid = :cid and LEFT(t1.sub_date,10) > '2024-10-23'");
    ...
    {
        $runtype = (($_SESSION['role'] == 2) ? 'DC' : 'RG');
        $this->db->query("SELECT * FROM `counselors` where status > 3 and received_by = :username and LEFT(sub_date,10) > '2024-10-23'");
        $this->db->bind(':username', $_SESSION['username']);
    ...
    {
        $runtype = (($_SESSION['role'] == 2) ? 'DC' : 'RG');
        $this->db->query("SELECT * FROM `counselors` where status in (1,2,3,4) and received_by = :username and LEFT(sub_date,10) > '2024-10-23'");
        $this->db->bind(':username', $_SESSION['username']);
    ...
    public function counsels_stat()
    {
        ...
        left join hrmscode.users as t2 on t1.received_by = t2.username where t1.received_by is not null and t1.status > 0 and LEFT(t1.sub_date,10) > '2024-10-23' GROUP BY received_by, fullname");
    ...
    public function reassigned()
    {
        ...
        left join osis.students_db as t2 on t1.studid = t2.studid
         where t1.received_by = :username and  t1.reassigned >0 and LEFT(t1.sub_date,10) > '2024-10-23'");
