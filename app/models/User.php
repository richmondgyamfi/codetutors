...
left join osis.students_db as t2 on t1.studid = t2.studid where t1.status = 0");
    ...
    public function counsel_received(){
        $this->db->query("SELECT t1.*, concat(t2.fname,' ',t2.lname) as sname FROM hrmscode.`counselors` as t1
left join osis.students_db as t2 on t1.studid = t2.studid where t1.status > 0 and t1.received_by = :username");
