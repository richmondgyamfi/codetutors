...
class Code_examiners extends Controller
{
    ...
    public function welcome()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // var_dump($_POST);die();
            // $checkstaff_no = $this->userModel->checkstaff_no($sno);
            $checkstaff_code = $this->userModel->checkstaff_code_pno($pno);
            // $checkstaff_code = $this->userModel->checkstaff_code($sno);
            // $checkstaff_no_hr = $this->userModel->checkstaff_no_hr($sno);
            $checkstaff_no_hr = $this->userModel->checkstaff_no_hr_pno($pno);

            // var_dump($checkstaff_no);die();
            ...
            if (!$checkstaff_code) {
                $data1 = [
                    'status' => 'error',
                    'message' => 'Could not find your data contact examination office'
                    // 'message' => 'Application has ended'
                ];
            } else {
                $_SESSION['staff_no_start'] = $pno;

                if (!$checkstaff_no_hr) {
                    $staffdata = $checkstaff_code;
                } else {
                    $staffdata = $checkstaff_no_hr;
                }
            ...
            $this->view('code_examiners/application');
        }
    }

    public function apply()
    {
        if (isset($_SESSION['staff_no_start'])) {
            // $sno = $_SESSION['staff_no_start'];
            $pno = $_SESSION['staff_no_start'];
            // $checkstaff_code = $this->userModel->checkstaff_code($sno);
            // $checkstaff_no_hr = $this->userModel->checkstaff_no_hr($sno);
            $checkstaff_code = $this->userModel->checkstaff_code_pno($pno);
            // $checkstaff_code = $this->userModel->checkstaff_code($sno);
            ...
            if (!$checkstaff_no_hr) {
                ...
                if (!empty($checkstaff_code[0]->fname)) {
                    $staffdata = $checkstaff_code;
                } else {
            ...
            $checkapply = $this->userModel->checkapply();
            // echo count($checkapply);die();
            $data = [
                'staffdata' => $checkapply
            ];
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($_POST['fname']) || empty($_POST['lname'])) {
                ...
                $data = [
                    ...
                    'ghcard_no' => strtoupper(trim($_POST['ghcard'])),
                    ...
                    'acc_name' => strtoupper(trim($_POST['acc_name'])),
                    'staffno' => trim($_POST['staffno']),
                    'acc_no' => strtoupper(trim($_POST['acc_no']))
                ];
    ...
    public function print()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($_POST['phone_no'])) {
                ...
                if ($checkapp) {
                    $this->userModel->code_logs($_POST['phone_no'], $activity);
                    ...
                    $data1 = [
                        'status' => 'success',
                        'message' => 'You can continue to print your application letter'
                    ];
                } else {
                    $data1 = [
                        'status' => 'error',
